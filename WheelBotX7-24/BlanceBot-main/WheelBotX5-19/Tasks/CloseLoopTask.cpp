...
void CloseLoopThreadFun(ULONG initial_input)
{
	...
	for(;;)
	{
		...
		else if(RobotControl->CheckRobotMode()==ROBOTMODE_ESCAPE)
		{
			...
			RobotControl->ChasisControl.LoopEscape[0].PID_Cal(RobotControl->ChasisControl.MotorUnits->KF9025[0].GetSpeed()*0.1);  
			RobotControl->ChasisControl.LoopEscape[1].PID_Cal(RobotControl->ChasisControl.MotorUnits->KF9025[1].GetSpeed()*0.1);
		...
		else
		{
			...
			yaw_ddwrNwwr = -powf(INS->Gyro[2],2)*CENTER_IMU_L+INS->dGyro[2]*CENTER_IMU_W;
			yaw_p_ddwrNwwr = -powf(INS->Gyro[1],2)*CENTER_IMU_W-INS->dGyro[1]*CENTER_IMU_L;
			pitch_ddwrNwwr = -powf(INS->Gyro[1],2)*CENTER_IMU_H+INS->dGyro[1]*CENTER_IMU_L;
			static float macc_y,macc_z;
			macc_y = INS->MotionAccel_b[0]+yaw_ddwrNwwr+yaw_p_ddwrNwwr;
//			RobotControl->ChasisControl.ObserveVal.X[4] = macc_y;
			macc_z = INS->MotionAccel_b[2]+pitch_ddwrNwwr;
//			RobotControl->ChasisControl.ObserveVal.X[5] =macc_z*arm_sin_f32(-RobotControl->ChasisControl.ObserveVal.ChasisPitch);
			RobotControl->ChasisControl.ObserveVal.X[5] =macc_y*arm_cos_f32(-RobotControl->ChasisControl.ObserveVal.ChasisPitch)-macc_z*arm_sin_f32(-RobotControl->ChasisControl.ObserveVal.ChasisPitch);
...
/**/  realaccelX = ((-RobotControl->INS->Accel[0]+RobotControl->INS->Accel[2])+9.8f*(SinPhi-CosPhi))/(SinPhi+CosPhi);
			...
			//求线速度
			RobotControl->ChasisControl.ObserveVal.X[0] = RobotControl->ChasisControl.ObserveVal.X[0]*0.100;
			RobotControl->ChasisControl.ObserveVal.X[1] = RobotControl->ChasisControl.ObserveVal.X[1]*0.100;
			//考虑腿打滑
			...
			RobotControl->ChasisControl.VelKF->UpdateKalman(RobotControl->ChasisControl.ObserveVal.X[3], RobotControl->ChasisControl.ObserveVal.X[5]);
...
//				if(RobotControl->ChasisControl.Lleg_ObserveVal.ChasisFn<18.0f&&RobotControl->ChasisControl.Rleg_ObserveVal.ChasisFn<18.0f)
//				{
...
//			RobotControl->ChasisControl.LoopYaw.PID_Cal(RobotControl->ChasisControl.ObserveVal.ChasisPsaiYaw);
  		...
  		RobotControl->ChasisControl.LoopGyro.PID_Cal(INS->Gyro[2]);
			//双腿都离地
			if(RobotControl->ChasisControl.CheckOG()==1)
			{
				//落地后快速回到 目标状态
					RobotControl->ChasisControl.Lleg_TargetVal.X[2] = RobotControl->ChasisControl.Lleg_ObserveVal.X[2];
					RobotControl->ChasisControl.Rleg_TargetVal.X[2] = RobotControl->ChasisControl.Rleg_ObserveVal.X[2];	
...
//					 RobotControl->ChasisControl.Rleg_TargetVal.X[0]=0.300f;
					...
					*/ 
					RobotControl->ChasisControl.SetLegLen(LEG_LEN4);
					RobotControl->ChasisControl.Set_GravityForward(15.0f);
					RobotControl->ChasisControl.Set_FlyMid_Flag(1);
				}
				else if(RobotControl->ChasisControl.Get_MoveMode()==Move_Normal)
				{
					RobotControl->ChasisControl.SetLegLen(LEG_LEN4);
					RobotControl->ChasisControl.Set_GravityForward(15.0f);
					RobotControl->ChasisControl.Set_FlyMid_Flag(1);
...
//				RobotControl->ChasisControl.Rleg_TargetVal.X[0]=0.000f;	
        RobotControl->ChasisControl.Set_GravityForward(71.0f);       					
				...
				{
				  RobotControl->ChasisControl.Lleg_TargetVal.X[0]=-0.15f;
				  RobotControl->ChasisControl.Rleg_TargetVal.X[0]=-0.15f;						
				}
				else
				{		
...
//				RobotControl->ChasisControl.Rleg_TargetVal.X[0]=0.3f;	
        ...
        RobotControl->ChasisControl.Set_GravityForward(45.0f);       					
			}
			else
			{	
				//给左右点击轮加上yaw转向输出
				...
				RobotControl->ChasisControl.Set_GravityForward(70.0f);
				if(RobotControl->ChasisControl.Get_MoveMode()==Move_Fly)
				{
...
//			RobotControl->ChasisControl.Lleg_LQR.RefreshLQRK(RobotControl->ChasisControl.MotorUnits->LinkSolver[0].GetPendulumLen(),1);
//			RobotControl->ChasisControl.Lleg_LQR.LQRCal(Lleg_Tlqr);			
//			RobotControl->ChasisControl.Rleg_LQR.RefreshLQRK(RobotControl->ChasisControl.MotorUnits->LinkSolver[1].GetPendulumLen(),1);
//			RobotControl->ChasisControl.Rleg_LQR.LQRCal(Rleg_Tlqr);			
			/*Wheel Banlance*/
			//调腿长  关掉LQR
...
//			RobotControl->ChasisControl.LoopLen_Dot[0].PID_Cal(-RobotControl->ChasisControl.Lleg_ObserveVal.ChasisLenDot);
//			RobotControl->ChasisControl.LoopLen_Dot[1].PID_Cal(RobotControl->ChasisControl.Rleg_ObserveVal.ChasisLenDot);
      ...
      //添加roll环
			...
			{	
			    ...
			    FT_L[0] = -(RobotControl->ChasisControl.LoopLen[0].GetOut()-RobotControl->ChasisControl.LoopLen_Dot[0].GetOut()+RobotControl->ChasisControl.Get_GravityForward());
			    FT_R[0] = -(RobotControl->ChasisControl.LoopLen[1].GetOut()+RobotControl->ChasisControl.LoopLen_Dot[1].GetOut()+RobotControl->ChasisControl.Get_GravityForward());			
