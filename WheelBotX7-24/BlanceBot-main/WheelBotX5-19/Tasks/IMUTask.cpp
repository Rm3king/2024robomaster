...
ULONG tim1x,tim2x;
uint8_t WhoAmI = 0;	uint8_t IMUError = 0;
void INSThreadFun(ULONG initial_input)
{
	...
	{
		timer = tx_time_get();
    const float gravity[3] = {0,0,9.8011f};
		//GYROCal();/*calibiration*/
		if(INS->Accel[0]==AX_Last)
		{
			if(++IMUError==50)
			{
				IMU_Init();
		...
		{IMUError=0;}
		AX_Last = INS->Accel[0];
    float dt = 0.001f; // 假设采样时间间隔为1ms
    float alpha = 0.9f; // 滤波系数

    for (int i = 0; i < 3; i++) {
        float dGyro = (INS->Gyro[i] - INS->PrevGyro[i]) / dt;
        INS->dGyro[i] = alpha * dGyro + (1 - alpha) * INS->dGyro[i];
        INS->PrevGyro[i] = INS->Gyro[i];
    }
		/*EKF迭代四元数*/
        IMU_QuaternionEKF_Update(INS->Q, -INS->Gyro[0], -INS->Gyro[1], INS->Gyro[2], -INS->Accel[0], -INS->Accel[1], INS->Accel[2]);
				float gravity_b[3];
		EarthFrameToBodyFrame(gravity, gravity_b, INS->Q);
    INS->MotionAccel_b[0] = 0.8*(-INS->Accel[0]-gravity_b[0])+0.2*INS->MotionAccel_b[0];
    INS->MotionAccel_b[1] = 0.8*(-INS->Accel[1]-gravity_b[1])+0.2*INS->MotionAccel_b[1];	
    INS->MotionAccel_b[2] = 0.8*(INS->Accel[2]-gravity_b[2])+0.2*INS->MotionAccel_b[2];			
		BodyFrameToEarthFrame(INS->MotionAccel_b,INS->MotionAccel_n,INS->Q);
		tx_thread_sleep_until(&timer,1);
	}
}
...
static void GYROCal(void)
{
	...
	else
	{
		...
		calval[0] = -(double)GYROCC[0]*INS->IMU->LSB_ACC_GYRO[1]/60000.0f;
		calval[1] = -(double)GYROCC[1]*INS->IMU->LSB_ACC_GYRO[1]/60000.0f;
		calval[2] = -(double)GYROCC[2]*INS->IMU->LSB_ACC_GYRO[1]/60000.0f;
