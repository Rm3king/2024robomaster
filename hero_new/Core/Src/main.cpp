...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
  static uint8_t tim2clk;
	static float BuffPower_ERR;
	static float BuffPower_I_Sum;
	static float power_kp_value,power_ki_value;
	static uint16_t power_plimit;
  static float power_kp = 0;
 ...
 if(tim2clk % 40 == 0) 
    {
      robot.getAttitudeSolutionTaskPointer()->getPIDGyrotempTaskPointer()->update(1e6f/5);
			if(robot.getRemoteControlTask()->open_flag ==1 )
			{
				__HAL_TIM_SetCompare(&htim1, TIM_CHANNEL_1, 1500);
				robot.getRemoteControlTask()->open_flag = 0;
			}
			if(robot.getRemoteControlTask()->close_flag ==1 )
			{
				__HAL_TIM_SetCompare(&htim1, TIM_CHANNEL_1, 550);
				robot.getRemoteControlTask()->close_flag = 0;
			}
			if(robot.getRefereeSystemTaskPointer()->getRobotRefereeStatus_t().power_heat_data.chassis_power>20)
			{
				BuffPower_ERR = robot.getRefereeSystemTaskPointer()->getRobotRefereeStatus_t().power_heat_data.chassis_power_buffer - 55.0f;
				
				power_kp_value = 4.0f * BuffPower_ERR;
				
//				power_kp += 5.0f * BuffPower_ERR;
				
				BuffPower_I_Sum += BuffPower_ERR * 0.1f;
				
				if(BuffPower_I_Sum> 200) BuffPower_I_Sum = 200;
				if(BuffPower_I_Sum< -200) BuffPower_I_Sum = -2500;
				
				power_ki_value = 0.03f * BuffPower_I_Sum;
				
				power_kp = power_kp + power_kp_value + power_ki_value;
				
//			if(robot.getRefereeSystemTaskPointer()->getRobotRefereeStatus_t().power_heat_data.chassis_power_buffer>50)
//			{
//				power_kp +=5.0f * __fabs();
//			}
//			else if(robot.getRefereeSystemTaskPointer()->getRobotRefereeStatus_t().power_heat_data.chassis_power_buffer<=50)
//			{
//				power_kp -=5.0f;
//			}
			if(power_kp>4) power_kp=4.0f;
			else if(power_kp<-7) power_kp = -7.0f;
		 }	
				power_plimit =  robot.getRefereeSystemTaskPointer()->getRobotRefereeStatus_t().game_robot_status.chassis_power_limit+power_kp;
				
			*(robot.getChassisControlTaskPointer()->get_can_tx_data()) = power_plimit;
    }
    ...
    if(tim2clk % 3 == 0) 
    {
	 ...
	 @brief 100HZ 调试窗口
		*/
		if(tim2clk%1==0)//100HZ
		{
				USART1_DMA_Debug_Plot(1,(int16_t)robot.getAmmoRM_ControlTaskPointer()->getBoosterRightPIDControlTaskPointer()->getmotor_backend_p_Pointer()->getMeasurement().speed_rpm);	

//			USART1_DMA_Debug_Plot(1,(int16_t)robot.getAmmoRM_ControlTaskPointer()->getBoosterRightPIDControlTaskPointer()->getmotor_backend_p_Pointer()->getMeasurement().speed_rpm);	
//			USART1_DMA_Debug_Plot(2,(int16_t)robot.getAmmoRM_ControlTaskPointer()->getBoosterLeftPIDControlTaskPointer()->getmotor_backend_p_Pointer()->getMeasurement().speed_rpm)
			;
...
//		if(rc_protocol == NULL) return;
  ...
  if(huart1.Instance->SR & UART_FLAG_IDLE)
  {
    ...
    start_pos = end_pos;
    end_pos = CV_FIFO_BUF_LEN - __HAL_DMA_GET_COUNTER(&hdma_usart1_rx);
    ComputerVisionControlTask *task_p = robot.getCV_ControlTaskPointer();
    if(task_p != NULL)
    {
      task_p->pushToBuffer(start_pos, end_pos);
    }
