...
void USART1_Send_String(const uint8_t * str)
{
...
void USART1_Debug_Printf(const char *format, ...)
{
	#ifdef DEBUG_MODE
  static uint8_t buf[256] = {0};
  static va_list ap;
...
void UART1_Debug_Plot(uint8_t name, int16_t value)
{
  char buf[20];
  sprintf(buf, "Line%d=%d,", name, value);
  USART1_Debug_Printf(buf);
}
